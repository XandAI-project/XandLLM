...
pub async fn run(
    ...
    if raw || !matches!(chat_fmt.as_str(), "chatml" | "qwen2" | "qwen3" | "chatml-thinking") {
    ...
    let stop_strings: Vec<String> = if raw {
        vec![]
    } else {
        xandllm_core::chat_template::stop_text_strings_for_format(&chat_fmt)
            .iter()
            .map(|s| s.to_string())
            .collect()
    };
